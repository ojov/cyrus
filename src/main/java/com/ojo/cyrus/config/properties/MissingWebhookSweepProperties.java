 ...
 * @param logSampleItem when true (the default), the first item scanned each run is logged in full —
 *                      regardless of whether it's a gap — so the real response schema can be
 *                      inspected even on a run with zero gaps (e.g. webhook delivery has been
 *                      reliable so far). Turn off once the schema is confirmed; it's log noise beyond
 *                      that point.
 */
...
public record MissingWebhookSweepProperties(
        ...
        @DefaultValue("true") boolean logSampleItem
) {}
