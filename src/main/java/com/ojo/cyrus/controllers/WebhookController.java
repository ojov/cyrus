...
import com.ojo.cyrus.services.NombaClient;
import com.ojo.cyrus.services.WebhookService;
...
public class WebhookController {
    ...
    public ResponseEntity<CyrusApiResponse<String>> receiveNombaWebhook(
        ...
        try {
            ...
            return ResponseEntity.accepted().body(
                CyrusApiResponse.success("Webhook accepted for processing")
            );
            ...
            return ResponseEntity.accepted().body(
                CyrusApiResponse.error("Internal error, webhook queued for retry")
            );
        }
    }
    ...
    public ResponseEntity<CyrusApiResponse<String>> webhookHealth() {
        return ResponseEntity.ok(
            CyrusApiResponse.success("Webhook endpoint is healthy")
        );
    }
    ...
    public ResponseEntity<CyrusApiResponse<WebhookStatusResponse>> getWebhookStatus() {
        try {
            ...
            return ResponseEntity.ok(CyrusApiResponse.success(status));

        } catch (Exception e) {
            log.error("Error getting webhook status", e);
            return ResponseEntity.internalServerError().body(
                CyrusApiResponse.error("Failed to get webhook status")
            );
        }
    }
    ...
    public ResponseEntity<CyrusApiResponse<String>> replayWebhook(
        ...
        try {
            ...
            return ResponseEntity.accepted().body(
                CyrusApiResponse.success("Webhook queued for replay")
            );

        } catch (Exception e) {
            log.error("Error replaying webhook", e);
            return ResponseEntity.internalServerError().body(
                CyrusApiResponse.error("Failed to replay webhook")
            );
        }
    }
}
