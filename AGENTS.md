...
- `exception/` — `GlobalExceptionHandler` (`@RestControllerAdvice`) + typed exceptions (incl. `InvalidWebhookUrlException` → 400, `InvalidPaymentEventStateException` → 409); return errors through it, not ad-hoc responses.
