...
- `models/entities/` — JPA entities: `Merchant`, `Customer`, `VirtualAccount`, `ApiKey`, `Transaction`, `PaymentEvent`, `MerchantWebhookEvent`, `VerificationToken` (all extend `BaseEntity`; `AuditConfig` provides JPA auditing).
...
- `enums/` — domain enums: `KycTier`, `MerchantStatus`, `VirtualAccountStatus`, `EventStatus`, `MatchStatus`, `TransactionStatus`, `ReconciliationOutcome`, `MerchantWebhookStatus`, `MerchantWebhookEventType`, `ApiKeyStatus`, `Environment`, `Provider`, `ResponseCode`.
