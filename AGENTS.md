...
- **Error contract:** every failure returns a `CyrusApiResponse` whose `data` is an `ErrorDetails` (reason + user-friendly message, optional `fieldErrors`, and a `traceId` on 5xx). Handlers log the actual throwable and return a **client-safe message** — never leak stack traces or raw provider payloads. Client (4xx) errors log the exception at `warn`; server/upstream (5xx) errors log at `error` with a short `traceId` that is echoed to the caller for support correlation. **Add a handler here rather than catching in services**, and don't log-before-throwing in services: the handler always logs the throwable, so a bare `throw` is enough to be diagnosable.
